...
**症状**: 縦書きでルビ付き漢字がごくまれに直上の文字に 1〜2px 食い込んで見える。
三四郎.epub の「私は癧症（かんしょう）」で確認。ほとんどの文字は問題なし。

**調査済み内容**:
- レイアウト座標は正しい（probe テストで `私=206, は=228, 癧=251` と確認、重なりなし）
- テスト環境のスクリーンショットでも正常描画を確認
- `FORMATTING_VERSION_ID 0x0042` にバンプ済み（古いキャッシュは自動無効化）
- アノテーションのセンタリングクランプ修正済み（annotation > base 時に上方向ブリード防止）

**根本原因（未解決）**: ルビグループのインラインボックスが基文字直前の体文字
（`は` など）の終端 y と 0〜1px しか離れていない。アノテーション長 > 基文字長
（例: かんしょう=45px > 癧症=44px）の場合、センタリングにより `か` が `は` の
末尾 y と同 y に配置される。グリフのアンチエイリアスや font metrics のはみ出しで
視覚的に重なって見える可能性がある。JLReq 的には 0.5px のオーバーハング余白が
必要だが現実装では未対応。

**影響範囲**: アノテーション文字数 > 基文字文字数 の ruby グループのみ、かつ
1px 単位の差のため非常にまれ。大半のルビは問題なし。
